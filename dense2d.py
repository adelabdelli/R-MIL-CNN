...
class Dense2D(Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.input_spec = InputSpec(min_ndim=2)                         #changes are made here
        self.supports_masking = True
        self.mil_function = mil_function
    ...
    def build(self, input_shape):
        ...
        input_dim1 = input_shape[-1] #changes are made here
        input_dim2 = input_shape[-2] #changes are made here
        if last_dim is None:
            raise ValueError(
        ...
        self.input_spec = InputSpec(min_ndim=2, axes={-1: last_dim})      #changes are made here
        self.kernel = self.add_weight(
            "kernel",
            shape=[input_dim2, input_dim1,  self.units],                                     #changes are made here
        ...
        if self.use_bias:
            self.bias = self.add_weight(
                "bias",
                #shape=(input_dim2, self.units),                                             #changes are made here
                shape=(self.units),
    ...
    def call(self, inputs):
        ...
        # Broadcast kernel to inputs.
            ...
            outputs = tf.einsum('...ij,ijk->...jk', inputs, self.kernel)
            #print("output after tensordot/tf.einsum:",outputs.shape)
            # Reshape the output back to the original ndim of the input.
            if not tf.executing_eagerly():
                shape = inputs.shape.as_list()
                #print("shape:",shape)
                #print("shape[:-1]:",shape[:-1])
                output_shape = [shape[0],shape[-1]] + [self.kernel.shape[-1]]
                #output_shape = shape[:-1] + [self.kernel.shape[-1]]
                #print("output2:",output_shape)
        ...
        if self.activation is not None:
            ...
            #print("after activation funcion the output shape is: ",outputs.shape)
            #ouputs = MIL_aggregation(outputs)
            outputs = self.mil_function(outputs)
            #print("mil function is applied")
            #print("after mil funcion the output shape is: ",outputs.shape)

        if is_ragged:
            outputs = original_inputs.with_flat_values(outputs)
            #print("ragged:",outputs.shape)
